...
public class ModBiomeModifiers {
    ...
    public static void bootstrap(BootstrapContext<BiomeModifier> context) {
        ...
        context.register(ADD_END_VOIDITE_ORE, new BiomeModifiers.AddFeaturesBiomeModifier(
                biomes.getOrThrow(BiomeTags.IS_END),
                HolderSet.direct(placedFeature.getOrThrow(ModPlacedFeatures.END_VOIDITE_ORE_PLACED_KEY)),
                GenerationStep.Decoration.UNDERGROUND_ORES));

      /*  context.register(ADD_GLOWING_VOID_BERRY_BUSH, new BiomeModifiers.AddFeaturesBiomeModifier(
                HolderSet.direct(biomes.getOrThrow(ModBiomes.SUNCROWN_FOREST)),
                HolderSet.direct(placedFeature.getOrThrow(ModPlacedFeatures.GLOWING_VOID_BERRY_BUSH_PLACED_KEY)),
                GenerationStep.Decoration.VEGETAL_DECORATION));

        context.register(ADD_VOID_GRASS_PATCH, new BiomeModifiers.AddFeaturesBiomeModifier(
                HolderSet.direct(biomes.getOrThrow(ModBiomes.SUNCROWN_FOREST)),
                HolderSet.direct(placedFeature.getOrThrow(ModPlacedFeatures.VOID_GRASS_PATCH_PLACED_KEY)),
                GenerationStep.Decoration.VEGETAL_DECORATION));

        context.register(ADD_END_SLUDGE_DISK, new BiomeModifiers.AddFeaturesBiomeModifier(
                HolderSet.direct(biomes.getOrThrow(ModBiomes.SUNCROWN_FOREST)),
